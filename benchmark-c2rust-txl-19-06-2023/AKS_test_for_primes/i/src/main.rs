...
pub extern "C" fn show(mut n: i32) {
    unsafe {
        loop {
            printf(b"%+lldx^%d\0" as *const u8 as *const i8, c[n as usize], n);
            let fresh1 = n;
            n = n - 1;
            if !(fresh1 != 0) {
                break;
...
fn main_0() -> i32 {
    ...
    unsafe {
        while n < 10 {
            coef(n);
            printf(b"(x-1)^%d = \0" as *const u8 as *const i8, n);
            show(n);
            print!("{}", '\n' as i32);
            n += 1;
            n;
        }
        printf(b"\nprimes (never mind the 1):\0" as *const u8 as *const i8);
    }
    n = 1;
    unsafe {
        while n <= 63 {
            if is_prime(n) != 0 {
                printf(b" %d\0" as *const u8 as *const i8, n);
            }
            n += 1;
            n;
        }
