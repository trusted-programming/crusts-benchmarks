...
pub extern "C" fn say_hundred(
    ...
    unsafe {
        ...
        if c[0 as usize] != 0 {
            printf(
                b"%s hundred\0" as *const u8 as *const i8,
                ones[c[0 as usize] as usize],
            );
            has_lead = 1;
        }
        if has_lead != 0 && (c[1 as usize] != 0 || c[2 as usize] != 0) {
            if (depth == 0 || c[0 as usize] != 0) && (c[0 as usize] == 0 || c[1 as usize] == 0) {
                printf(b"and \0" as *const u8 as *const i8)
            } else {
                if c[0 as usize] != 0 {
        ...
        if c[1 as usize] < 2 {
            if c[1 as usize] != 0 || c[2 as usize] != 0 {
                printf(
                    ...
                    ones[(c[1 as usize] * 10 + c[2 as usize]) as usize],
                );
            }
        } else {
            if c[1 as usize] != 0 {
    ...
    unsafe {
        ...
        loop {
            if say_hundred(s, r, n, has_lead) != 0 && n != 0 {
                has_lead = 1;
                printf(b" %s\0" as *const u8 as *const i8, llions[n as usize]);
                if depth == 0 {
                    printf(b", \0" as *const u8 as *const i8);
                } else {
                    printf(b" \0" as *const u8 as *const i8);
            ...
            14523784380283086299 => {
                ...
                if !(len == 0) {
                    ...
                    while i < len {
                        ...
                        {
                            printf(b"(not a number)\0" as *const u8 as *const i8);
                            return;
                        }
                        i += 1;
                        i;
                    ...
                    if got_sign == -1 {
                        printf(b"minus \0" as *const u8 as *const i8);
                    }
                    n = len / maxillion;
                    r = len % maxillion;
                    if r == 0 {
                    ...
                    loop {
                        ...
                        if has_lead != 0 {
                            ...
                            while i < n {
                                printf(
                                    b" %s\0" as *const u8 as *const i8,
                                    llions[(maxillion / 3i32) as usize],
                                );
                                i += 1;
                                i;
                            }
