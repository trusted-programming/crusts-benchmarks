    ...
    unsafe {
        ...
        while i < nLen {
            printf(
                ...
                *pbyBin.offset(i as isize) as i32,
            );
            i += 1;
            i;
        }
...
pub extern "C" fn testcase(mut pszTest: *const i8) {
    unsafe {
        ...
        printf(b"Test case '%s'\n\0" as *const u8 as *const i8, pszTest);
        ...
        printf(b"addr:  \0" as *const u8 as *const i8);
        dumpbin(abyAddr.as_mut_ptr(), if bIsIPv6 != 0 { 16 } else { 4 });
        printf(b"\n\0" as *const u8 as *const i8);
        if 0 == nPort {
            printf(b"port absent\0" as *const u8 as *const i8);
        } else {
            printf(
                b"port:  %d\0" as *const u8 as *const i8,
                htons(nPort as u16) as i32,
            );
        }
        printf(b"\n\n\0" as *const u8 as *const i8);
    }
