...
pub extern "C" fn printout(mut balls: *mut i8) {
    unsafe {
        ...
        printf(b"%s\n\0" as *const u8 as *const i8, str.as_mut_ptr());
    }
}
...
fn main_0() -> i32 {
    ...
    unsafe {
        ...
        while issorted(balls.as_mut_ptr()) {
            printf(b"Accidentally still sorted: \0" as *const u8 as *const i8);
        ...
        printf(b"Non-sorted: \0" as *const u8 as *const i8);
    }
    printout(balls.as_mut_ptr());
    unsafe {
        qsort(
        ...
        if issorted(balls.as_mut_ptr()) {
            printf(b"Sorted: \0" as *const u8 as *const i8);
            printout(balls.as_mut_ptr());
        } else {
            printf(b"Sort failed: \0" as *const u8 as *const i8);
            printout(balls.as_mut_ptr());
        }
    }
    return 0;
