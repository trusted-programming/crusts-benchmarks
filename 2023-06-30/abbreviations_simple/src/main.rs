...
pub extern "C" fn command_match(mut command: *const command_t, mut str: *const i8) -> bool {
    unsafe {
        ...
        olen >= (*command).min_len
            && olen <= (*command).length
            && strncmp(str, (*command).cmd, olen) == 0_i32
    }
}
...
pub extern "C" fn uppercase(mut str: *mut i8, mut n: u64) -> *mut i8 {
    unsafe {
        let mut i: u64 = 0;
        while i < n {
            *str.offset(i as isize) = toupper(i32::from(*str.offset(i as isize))) as i8;
            i = i.wrapping_add(1);
            i;
        }
        str
...
pub extern "C" fn xmalloc(mut n: u64) -> *mut libc::c_void {
    unsafe {
        ...
        if ptr.is_null() {
            fatal((b"Out of memory\0" as *const u8).cast::<i8>());
        }
        ptr
    }
}
...
pub extern "C" fn split_into_words(mut str: *const i8, mut count: *mut u64) -> *mut *mut i8 {
    unsafe {
        ...
        let mut words: *mut *mut i8 =
            xmalloc(capacity.wrapping_mul(::core::mem::size_of::<*mut i8>() as u64)).cast::<*mut i8>();
        let mut len: u64 = strlen(str);
        let mut begin: u64 = 0;
        while begin < len {
            let mut i: u64 = begin;
            while i < len
                && i32::from(*(*__ctype_b_loc()).offset(i32::from(*str.offset(i as isize)) as isize))
                    & _ISspace as i32
                    != 0_i32
            {
                i = i.wrapping_add(1);
            ...
            let mut word: *mut i8 = xmalloc(word_len.wrapping_add(1)).cast::<i8>();
            memcpy(
                word.cast::<libc::c_void>(),
                str.offset(begin as isize).cast::<libc::c_void>(),
                word_len,
            );
            *word.offset(word_len as isize) = 0;
            begin = (begin).wrapping_add(word_len);
            if capacity == size {
                capacity = (capacity).wrapping_mul(2);
                words = xrealloc(
                    words.cast::<libc::c_void>(),
                    capacity.wrapping_mul(::core::mem::size_of::<*mut i8>() as u64),
                ).cast::<*mut i8>();
            }
            let fresh0 = size;
            size = size.wrapping_add(1);
            let fresh1 = &mut (*words.offset(fresh0 as isize));
            *fresh1 = word;
        }
        *count = size;
        words
    ...
    unsafe {
        ...
        while i < count {
            ...
            let mut new_cmd: *mut command_t =
                xmalloc(::core::mem::size_of::<command_t>() as u64).cast::<command_tag>();
            ...
            if i.wrapping_add(1) < count {
                let mut eptr: *mut i8 = std::ptr::null_mut::<i8>();
                let mut min_len: u64 =
                    strtoul(*words.offset(i.wrapping_add(1) as isize), &mut eptr, 10);
                if min_len > 0 && i32::from(*eptr) == 0_i32 {
                    free((*words.offset(i.wrapping_add(1) as isize)).cast::<libc::c_void>());
...
pub extern "C" fn free_command_list(mut cmd: *mut command_t) {
    unsafe {
        while !cmd.is_null() {
            ...
            free((*cmd).cmd.cast::<libc::c_void>());
            free(cmd.cast::<libc::c_void>());
            cmd = next;
        }
    }
}
...
fn main_0() -> i32 {
    unsafe {
        ...
        let mut input: *const i8 =
            (b"riG   rePEAT copies  put mo   rest    types   fup.    6       poweRin\0" as *const u8).cast::<i8>();
        test(commands, input);
