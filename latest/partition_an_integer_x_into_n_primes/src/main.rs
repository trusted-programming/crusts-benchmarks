...
// SAFETY: machine generated unsafe code
    unsafe {
        let mut array: *mut u32 = calloc(
            u64::from(size.wrapping_add(31).wrapping_div(32)),
...
// SAFETY: machine generated unsafe code
    unsafe {
        if index < (*b).size {
            ...
            __assert_fail(
                (b"index < b->size\0" as *const u8).cast::<i8>(),
...
// SAFETY: machine generated unsafe code
    unsafe {
        ...
        let mut bit: u32 = (1i32 << (index & 31)) as u32;
        *((*b).array).offset((index >> 5i32) as isize) & bit != 0
    }
}
...
// SAFETY: machine generated unsafe code
    unsafe {
        ...
        bit_array_set(&mut (*s).not_prime, 0, 1_i32 != 0_i32);
        bit_array_set(&mut (*s).not_prime, 1, 1_i32 != 0_i32);
        let mut p: u32 = 2;
        while p.wrapping_mul(p) <= limit {
            if i32::from(bit_array_get(&mut (*s).not_prime, p)) == 0_i32 {
                let mut q: u32 = p.wrapping_mul(p);
                while q <= limit {
                    bit_array_set(&mut (*s).not_prime, q, 1_i32 != 0_i32);
                    q = (q).wrapping_add(p);
                }
            }
            p = p.wrapping_add(1);
...
// SAFETY: machine generated unsafe code
    unsafe {
        if n <= (*s).limit {
            ...
            __assert_fail(
                (b"n <= s->limit\0" as *const u8).cast::<i8>(),
                (b"main.c\0" as *const u8).cast::<i8>(),
                67,
        ...
        i32::from(bit_array_get(&(*s).not_prime, n)) == 0_i32
    }
}
...
// SAFETY: machine generated unsafe code
    unsafe {
        ...
        while prime < number {
            if is_prime(s, prime) && find_prime_partition(
                    s,
...
// SAFETY: machine generated unsafe code
    unsafe {
        if count > 0 {
            ...
            __assert_fail(
                (b"count > 0\0" as *const u8).cast::<i8>(),
                (b"main.c\0" as *const u8).cast::<i8>(),
                93,
        ...
        let mut primes: *mut u32 =
            malloc(u64::from(count).wrapping_mul(::core::mem::size_of::<u32>() as u64)).cast::<u32>();
        if primes.is_null() {
        ...
        if !find_prime_partition(s, number, count, 2, primes) {
            ...
            print!("{} = {}", number, *primes.offset(0_isize));
            let mut i: u32 = 1;
            while i < count {
