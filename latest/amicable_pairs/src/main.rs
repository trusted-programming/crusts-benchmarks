...
// SAFETY: machine generated unsafe code
    unsafe {
        ...
        let mut divsum: *mut u32 = malloc(
            u64::from(top.wrapping_add(1u32)).wrapping_mul(::core::mem::size_of::<u32>() as u64),
        ).cast::<u32>();
        let mut pows: [u32; 32] = [
        ...
        while p.wrapping_add(p) <= top {
            if *divsum.offset(p as isize) > 1 {
                let fresh0 = &mut (*divsum.offset(p as isize));
                *fresh0 = (*fresh0 as u32).wrapping_sub(p) as u32;
                ...
                while n <= top {
                    let mut s: u32 = 1u32.wrapping_add(pows[1_usize]);
                    k = k.wrapping_sub(1);
                    k;
                    if k == 0 {
                        let mut i_0: u32 = 2;
                        while i_0 < x && n.wrapping_rem(pows[i_0 as usize]) == 0 {
                            ...
                            s = (s).wrapping_add(pows[fresh1 as usize]);
                        }
                        k = p;
                    }
                    let fresh2 = &mut (*divsum.offset(n as isize));
        ...
        while p_0 <= top {
            if *divsum.offset(p_0 as isize) > 1 {
                let fresh3 = &mut (*divsum.offset(p_0 as isize));
