    ...
    unsafe {
        if libzahl_set_up != 0 {
            ...
            loop {
                ...
                free(
                    (*(libzahl_tmp_divmod_ds[i as usize]).as_mut_ptr()).chars as *mut libc::c_void,
                );
            }
            i = (::std::mem::size_of::<[*mut *mut u32; 64]>() as u64)
                .wrapping_div(::std::mem::size_of::<*mut *mut u32>() as u64);
            loop {
                let fresh1 = i;
                i = i.wrapping_sub(1);
                if !(fresh1 != 0) {
