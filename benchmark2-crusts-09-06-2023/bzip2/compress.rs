...
#[inline]
extern "C" fn bsW(mut s: *mut EState, mut n: Int32, mut v: UInt32) {
    unsafe {
...
extern "C" fn bsPutUInt32(mut s: *mut EState, mut u: UInt32) {
    unsafe {
        bsW(
            s,
            8 as std::os::raw::c_int,
            ((u >> 24 as std::os::raw::c_int) as std::os::raw::c_long
                & 0xff as std::os::raw::c_long) as UInt32,
        );
        bsW(
            s,
            8 as std::os::raw::c_int,
            ((u >> 16 as std::os::raw::c_int) as std::os::raw::c_long
                & 0xff as std::os::raw::c_long) as UInt32,
        );
        bsW(
            s,
...
extern "C" fn bsPutUChar(mut s: *mut EState, mut c: UChar) {
    unsafe {
    ...
    unsafe {
        ...
        while i < (*s).nblock {
                ...
                == ll_i as std::os::raw::c_int
            {
                ...
                ryy_j =
                    &mut *yy.as_mut_ptr().offset(1 as std::os::raw::c_int as isize) as *mut UChar;
                rll_i = ll_i;
...
extern "C" fn sendMTFValues(mut s: *mut EState) {
    unsafe {
        ...
        while iter < 4 as std::os::raw::c_int {
            ...
            while 1 as std::os::raw::c_int as Bool != 0 {
                    ...
                    && 50 as std::os::raw::c_int == ge - gs + 1 as std::os::raw::c_int
                {
                    ...
                    cost01 = (cost01 as std::os::raw::c_uint).wrapping_add(
                        (*s).len_pack[icv as usize][0 as std::os::raw::c_int as usize],
                    ) as UInt32 as UInt32;
                    cost23 = (cost23 as std::os::raw::c_uint).wrapping_add(
                        (*s).len_pack[icv as usize][1 as std::os::raw::c_int as usize],
                    ) as UInt32 as UInt32;
                    cost45 = (cost45 as std::os::raw::c_uint).wrapping_add(
                        (*s).len_pack[icv as usize][2 as std::os::raw::c_int as usize],
                    ) as UInt32 as UInt32;
        ...
        if !(selCtr == nSelectors) {
            BZ2_bz__AssertH__fail(3007 as std::os::raw::c_int);
        }
        if (*s).verbosity >= 3 as std::os::raw::c_int {
            fprintf(
                __stderrp,
...
#[no_mangle]
pub extern "C" fn BZ2_compressBlock(mut s: *mut EState, mut is_last_block: Bool) {
    unsafe {
