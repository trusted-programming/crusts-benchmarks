        ...
        while sp > 0 as std::os::raw::c_int {
            ...
            if hi - lo < 10 as std::os::raw::c_int {
                ...
                if r3 == 0 as std::os::raw::c_int as std::os::raw::c_uint {
                    ...
                    med =
                        *eclass
                            .offset(*fmap.offset((lo + hi >> 1 as std::os::raw::c_int) as isize)
                                as isize)
                } else {
...
extern "C" fn fallbackSort(
    ...
    unsafe {
        ...
        while i < 256 as std::os::raw::c_int {
            ...
            *fresh0 |= (1 as std::os::raw::c_int as UInt32)
                << (ftab[i as usize] & 31 as std::os::raw::c_int);
            i += 1
        }
        i = 0 as std::os::raw::c_int;
        while i < 32 as std::os::raw::c_int {
            let ref mut fresh1 = *bhtab.offset(
                (nblock + 2 as std::os::raw::c_int * i >> 5 as std::os::raw::c_int) as isize,
            );
            *fresh1 |= (1 as std::os::raw::c_int as UInt32)
            ...
            *fresh2 &= !((1 as std::os::raw::c_int as UInt32)
                << (nblock + 2 as std::os::raw::c_int * i + 1 as std::os::raw::c_int
                ...
                if r > l {
                    ...
                    while i <= r {
                        ...
                        if cc != cc1 {
                            ...
                            *fresh3 |= (1 as std::os::raw::c_int as UInt32)
    ...
    unsafe {
        ...
        loop {
            ...
            if s1 as std::os::raw::c_int != s2 as std::os::raw::c_int {
                return (s1 as std::os::raw::c_int > s2 as std::os::raw::c_int)
                    as std::os::raw::c_int as Bool;
            }
            i1 = i1.wrapping_add(1);
            i2 = i2.wrapping_add(1);
...
#[inline]
extern "C" fn mmed3(mut a: UChar, mut b: UChar, mut c: UChar) -> UChar {
    let mut t: UChar = 0;
                ...
                med = mmed3(
                    ...
                    *block.offset(
                        (*ptr.offset((lo + hi >> 1 as std::os::raw::c_int) as isize))
                            .wrapping_add(d as std::os::raw::c_uint)
                            as isize,
                    ),
                ) as Int32;
                ltLo = lo;
...
extern "C" fn mainSort(
    ...
    unsafe {
        ...
        while i <= 65536 as std::os::raw::c_int {
            ...
            *fresh9 = (*fresh9 as std::os::raw::c_uint)
        ...
        loop {
            ...
            while i <= 255 as std::os::raw::c_int {
                ...
                .wrapping_sub(
                    *ftab.offset(
                        (runningOrder[(j - h) as usize] << 8 as std::os::raw::c_int) as isize,
                    ),
                ) > (*ftab
        ...
        while i <= 255 as std::os::raw::c_int {
            ...
            while j <= 255 as std::os::raw::c_int {
                if j != ss {
                    ...
                    *fresh10 |= ((1 as std::os::raw::c_int) << 21 as std::os::raw::c_int)
                        as std::os::raw::c_uint
                }
                j += 1
            }
            ...
            j = 0 as std::os::raw::c_int;
            while j <= 255 as std::os::raw::c_int {
                copyStart[j as usize] = (*ftab
                    .offset(((j << 8 as std::os::raw::c_int) + ss) as isize)
                    & !((1 as std::os::raw::c_int) << 21 as std::os::raw::c_int)
                        as std::os::raw::c_uint) as Int32;
                copyEnd[j as usize] = (*ftab.offset(
                    ((j << 8 as std::os::raw::c_int) + ss + 1 as std::os::raw::c_int) as isize,
                ) & !((1 as std::os::raw::c_int)
            ...
            j = 0 as std::os::raw::c_int;
            while j <= 255 as std::os::raw::c_int {
                let ref mut fresh13 = *ftab.offset(((j << 8 as std::os::raw::c_int) + ss) as isize);
                *fresh13 |= ((1 as std::os::raw::c_int) << 21 as std::os::raw::c_int)
                    as std::os::raw::c_uint;
                j += 1
            }
