...
#[no_mangle]
pub extern "C" fn ti_roc_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
...
pub extern "C" fn ti_roc(
    ...
    unsafe {
        ...
        if !(output.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            ...
            == (size - ti_roc_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 7], &[std::os::raw::c_char; 7]>(b"ti_roc\x00"))
                    .as_ptr(),
                b"indicators/roc.c\x00" as *const u8 as *const std::os::raw::c_char,
                46 as std::os::raw::c_int,
                b"output - outputs[0] == size - ti_roc_start(options)\x00" as *const u8
