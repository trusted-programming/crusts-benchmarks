...
#[no_mangle]
pub extern "C" fn ti_dx_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
    ...
    unsafe {
        ...
        *fresh0 = dx;
        ...
        if !(output.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            ...
            == (size - ti_dx_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 6], &[std::os::raw::c_char; 6]>(b"ti_dx\x00"))
                    .as_ptr(),
                b"indicators/dx.c\x00" as *const u8 as *const std::os::raw::c_char,
                101 as std::os::raw::c_int,
                b"output - outputs[0] == size - ti_dx_start(options)\x00" as *const u8
