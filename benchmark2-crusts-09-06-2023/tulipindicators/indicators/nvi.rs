...
#[no_mangle]
pub extern "C" fn ti_nvi_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
        return 0;
    }
}
    ...
    unsafe {
        ...
        *fresh0 = nvi;
        ...
        if !(output.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            ...
            == (size - ti_nvi_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 7], &[std::os::raw::c_char; 7]>(b"ti_nvi\x00"))
                    .as_ptr(),
                b"indicators/nvi.c\x00" as *const u8 as *const std::os::raw::c_char,
                56 as std::os::raw::c_int,
                b"output - outputs[0] == size - ti_nvi_start(options)\x00" as *const u8
