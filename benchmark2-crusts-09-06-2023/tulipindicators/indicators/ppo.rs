...
#[no_mangle]
pub extern "C" fn ti_ppo_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
        return 1;
    }
}
...
pub extern "C" fn ti_ppo(
    ...
    unsafe {
        ...
        while i < size {
            ...
            let out: std::os::raw::c_double = 100.0f64 * (short_ema - long_ema) / long_ema;
            let fresh0 = ppo;
            ppo = ppo.offset(1);
            *fresh0 = out;
            i += 1
        ...
        if !(ppo.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            ...
            == (size - ti_ppo_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 7], &[std::os::raw::c_char; 7]>(b"ti_ppo\x00"))
                    .as_ptr(),
                b"indicators/ppo.c\x00" as *const u8 as *const std::os::raw::c_char,
                63 as std::os::raw::c_int,
                b"ppo - outputs[0] == size - ti_ppo_start(options)\x00" as *const u8
