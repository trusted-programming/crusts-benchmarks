...
#[no_mangle]
pub extern "C" fn ti_natr_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
...
pub extern "C" fn ti_natr(
    ...
    unsafe {
        ...
        *fresh0 = 100 as std::os::raw::c_int as std::os::raw::c_double * val
        ...
        while i < size {
            ...
            let fresh1 = output;
            output = output.offset(1);
            *fresh1 = 100 as std::os::raw::c_int as std::os::raw::c_double * val
                / *close.offset(i as isize);
            i += 1
        }
        if !(output.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            as std::os::raw::c_long
            == (size - ti_natr_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 8], &[std::os::raw::c_char; 8]>(b"ti_natr\x00"))
                    .as_ptr(),
                b"indicators/natr.c\x00" as *const u8 as *const std::os::raw::c_char,
                69 as std::os::raw::c_int,
                b"output - outputs[0] == size - ti_natr_start(options)\x00" as *const u8
