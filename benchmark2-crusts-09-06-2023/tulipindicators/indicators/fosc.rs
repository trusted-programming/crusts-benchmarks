...
#[no_mangle]
pub extern "C" fn ti_fosc_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
    ...
    unsafe {
        ...
        while i < size {
            ...
            if i >= period {
                ...
                *fresh0 = 100 as std::os::raw::c_int as std::os::raw::c_double
        ...
        if !(output.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            ...
            == (size - ti_fosc_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 8], &[std::os::raw::c_char; 8]>(b"ti_fosc\x00"))
                    .as_ptr(),
                b"indicators/fosc.c\x00" as *const u8 as *const std::os::raw::c_char,
                54 as std::os::raw::c_int,
                b"output - outputs[0] == size - ti_fosc_start(options)\x00" as *const u8
