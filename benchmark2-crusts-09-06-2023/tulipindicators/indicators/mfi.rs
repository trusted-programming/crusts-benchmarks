...
#[no_mangle]
pub extern "C" fn ti_mfi_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
...
pub extern "C" fn ti_mfi(
    ...
    unsafe {
        ...
        let mut ytyp: std::os::raw::c_double = (*high.offset(0 as std::os::raw::c_int as isize)
        ...
        while i < size {
            ...
            if typ > ytyp {
                ...
                *(*up).vals.as_mut_ptr().offset((*up).index as isize) = bar;
                ...
                if (*up).index >= (*up).size {
                    (*up).index = 0 as std::os::raw::c_int
                }
                if (*down).pushes >= (*down).size {
                    (*down).sum -= *(*down).vals.as_mut_ptr().offset((*down).index as isize)
                };
                (*down).sum += 0.0f64;
        ...
        if !(output.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            ...
            == (size - ti_mfi_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 7], &[std::os::raw::c_char; 7]>(b"ti_mfi\x00"))
                    .as_ptr(),
                b"indicators/mfi.c\x00" as *const u8 as *const std::os::raw::c_char,
                78 as std::os::raw::c_int,
                b"output - outputs[0] == size - ti_mfi_start(options)\x00" as *const u8
