...
#[no_mangle]
pub extern "C" fn ti_adxr_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
...
pub extern "C" fn ti_adxr(
    ...
    unsafe {
            ...
            if i - period < period - 2 as std::os::raw::c_int {
                ...
                *(*adxr).vals.as_mut_ptr().offset((*adxr).index as isize) = adx * invper;
                ...
                if i >= first_adxr {
                    ...
                    *fresh0 = 0.5f64
                        * (adx * invper
                            + *(*adxr).vals.as_mut_ptr().offset(
                                (((*adxr).index + (*adxr).size - 1 as std::os::raw::c_int
                                    + 1 as std::os::raw::c_int)
                                    % (*adxr).size) as isize,
                            ))
                };
        ...
        if !(output.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            ...
            == (size - ti_adxr_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 8], &[std::os::raw::c_char; 8]>(b"ti_adxr\x00"))
                    .as_ptr(),
                b"indicators/adxr.c\x00" as *const u8 as *const std::os::raw::c_char,
                122 as std::os::raw::c_int,
                b"output - outputs[0] == size - ti_adxr_start(options)\x00" as *const u8
