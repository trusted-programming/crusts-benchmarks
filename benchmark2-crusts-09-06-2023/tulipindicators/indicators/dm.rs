...
#[no_mangle]
pub extern "C" fn ti_dm_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
...
pub extern "C" fn ti_dm(
    ...
    unsafe {
        ...
        let per: std::os::raw::c_double = (period as std::os::raw::c_double
        ...
        while i < period {
            ...
            dp = *high.offset(i as isize) - *high.offset((i - 1 as std::os::raw::c_int) as isize);
            dm = *low.offset((i - 1 as std::os::raw::c_int) as isize) - *low.offset(i as isize);
        ...
        *fresh1 = dmdown;
        ...
        while i < size {
            ...
            dp_0 = *high.offset(i as isize) - *high.offset((i - 1 as std::os::raw::c_int) as isize);
            dm_0 = *low.offset((i - 1 as std::os::raw::c_int) as isize) - *low.offset(i as isize);
        ...
        if !(plus_dm.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            as std::os::raw::c_long
            == (size - ti_dm_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 6], &[std::os::raw::c_char; 6]>(b"ti_dm\x00"))
                    .as_ptr(),
                b"indicators/dm.c\x00" as *const u8 as *const std::os::raw::c_char,
                77 as std::os::raw::c_int,
                b"plus_dm - outputs[0] == size - ti_dm_start(options)\x00" as *const u8
                    as *const std::os::raw::c_char,
            );
        } else {
        };
        if !(minus_dm.offset_from(*outputs.offset(1 as std::os::raw::c_int as isize))
            as std::os::raw::c_long
            == (size - ti_dm_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                ...
                b"minus_dm - outputs[1] == size - ti_dm_start(options)\x00" as *const u8
                    as *const std::os::raw::c_char,
            );
        } else {
        };
