...
#[no_mangle]
pub extern "C" fn ti_cmo_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
...
pub extern "C" fn ti_cmo(
    ...
    unsafe {
        ...
        let mut down_sum: std::os::raw::c_double =
            0 as std::os::raw::c_int as std::os::raw::c_double;
        let mut i: std::os::raw::c_int = 0;
        i = 1 as std::os::raw::c_int;
        while i <= period {
            up_sum += if *input.offset(i as isize)
            ...
            down_sum += if *input.offset(i as isize)
                < *input.offset((i - 1 as std::os::raw::c_int) as isize)
            {
        ...
        *fresh0 = 100 as std::os::raw::c_int as std::os::raw::c_double * (up_sum - down_sum)
            / (up_sum + down_sum);
        i = period + 1 as std::os::raw::c_int;
        while i < size {
            up_sum -= if *input.offset((i - period) as isize)
            ...
            down_sum -= if *input.offset((i - period) as isize)
                < *input.offset((i - period - 1 as std::os::raw::c_int) as isize)
            {
                (*input.offset((i - period - 1 as std::os::raw::c_int) as isize))
                    - *input.offset((i - period) as isize)
            ...
            let fresh1 = output;
            output = output.offset(1);
            *fresh1 = 100 as std::os::raw::c_int as std::os::raw::c_double * (up_sum - down_sum)
                / (up_sum + down_sum);
            i += 1
        }
        if !(output.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            as std::os::raw::c_long
            == (size - ti_cmo_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 7], &[std::os::raw::c_char; 7]>(b"ti_cmo\x00"))
                    .as_ptr(),
                b"indicators/cmo.c\x00" as *const u8 as *const std::os::raw::c_char,
                64 as std::os::raw::c_int,
                b"output - outputs[0] == size - ti_cmo_start(options)\x00" as *const u8
