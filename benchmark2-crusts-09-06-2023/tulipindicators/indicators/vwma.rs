...
#[no_mangle]
pub extern "C" fn ti_vwma_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
...
pub extern "C" fn ti_vwma(
    ...
    unsafe {
        ...
        *fresh0 = sum / vsum;
        ...
        while i < size {
            sum += *input.offset(i as isize) * *volume.offset(i as isize);
            sum -= *input.offset((i - period) as isize) * *volume.offset((i - period) as isize);
            vsum += *volume.offset(i as isize);
        ...
        if !(output.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            ...
            == (size - ti_vwma_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 8], &[std::os::raw::c_char; 8]>(b"ti_vwma\x00"))
                    .as_ptr(),
                b"indicators/vwma.c\x00" as *const u8 as *const std::os::raw::c_char,
                64 as std::os::raw::c_int,
                b"output - outputs[0] == size - ti_vwma_start(options)\x00" as *const u8
