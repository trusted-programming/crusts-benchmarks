...
#[no_mangle]
pub extern "C" fn ti_di_start(mut options: *const std::os::raw::c_double) -> std::os::raw::c_int {
    unsafe {
...
pub extern "C" fn ti_di(
    ...
    unsafe {
        ...
        *fresh1 = 100.0f64 * dmdown / atr;
        ...
        if !(plus_di.offset_from(*outputs.offset(0 as std::os::raw::c_int as isize))
            as std::os::raw::c_long
            == (size - ti_di_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                (*::std::mem::transmute::<&[u8; 6], &[std::os::raw::c_char; 6]>(b"ti_di\x00"))
                    .as_ptr(),
                b"indicators/di.c\x00" as *const u8 as *const std::os::raw::c_char,
                88 as std::os::raw::c_int,
                b"plus_di - outputs[0] == size - ti_di_start(options)\x00" as *const u8
                    as *const std::os::raw::c_char,
            );
        } else {
        };
        if !(minus_di.offset_from(*outputs.offset(1 as std::os::raw::c_int as isize))
            as std::os::raw::c_long
            == (size - ti_di_start(options)) as std::os::raw::c_long)
            as std::os::raw::c_int as std::os::raw::c_long
            != 0
        {
            __assert_rtn(
                ...
                b"minus_di - outputs[1] == size - ti_di_start(options)\x00" as *const u8
                    as *const std::os::raw::c_char,
            );
        } else {
        };
